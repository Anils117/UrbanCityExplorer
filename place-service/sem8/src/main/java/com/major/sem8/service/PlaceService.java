...
import com.major.sem8.dto.PlaceRequest;
...
//            return mapToDtoWithReviews(place,reviews.getBody());
    ...
    public PlaceResponse addPlace(PlaceRequest placeRequest) {
        // Validate placeRequest data
        if (placeRequest == null) {
            throw new IllegalArgumentException("Place request cannot be null");
        }

        // Create a new Place entity from the request data
        Place place = new Place();
        place.setPlaceName(placeRequest.getPlaceName());
        place.setDiscription(placeRequest.getDiscription());
        place.setAddress(placeRequest.getAddress());
        place.setMainImage(placeRequest.getMainImage());
        place.setCity(placeRequest.getCity());
        place.setCategory(placeRequest.getCategory());
        place.setTicketPrice(placeRequest.getTicketPrice());

        // Save the new place to the database
        Place savedPlace = placeRepository.save(place);

        // Map the saved place entity to a PlaceResponse DTO
